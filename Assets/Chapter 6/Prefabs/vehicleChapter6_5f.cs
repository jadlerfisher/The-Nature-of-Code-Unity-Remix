...
public class vehicleChapter6_5f : MonoBehaviour
{
    ...
    public void pathFollow(pathChapter65 path) {

        Vector3 predict = velocity;
        Vector3 pNormal = predict.normalized;
        pNormal *= 25;
        Vector3 predictedLocation = velocity + pNormal;

        Vector3 a = path.startVector;
        Vector3 b = path.endVector;
        Vector3 normalPoint = getNormalPoint(predictedLocation, a, b);

        Vector3 dir = b - a;
        Vector3 dirNormal = dir.normalized;
        dirNormal *= 1;
        Vector3 target = normalPoint + dirNormal;

        //Debug.Log(target + " this is the target");


        float distance = Vector3.Distance(normalPoint/1000f, predictedLocation);
        Debug.Log(normalPoint + "  normal point" + predictedLocation + "   " + distance + "    distance");
